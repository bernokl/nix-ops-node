...
#+begin_src tmux :session s1
ssh -i id_rsa.pem root@54.153.212.43
#+end_src
...
#+begin_src tmux :session s1
iptables -I INPUT 1 -s 75.8.76.164/32 -j ACCEPT
#+end_src
- Logged out of the machine and try:
#+begin_src tmux :session s1
nix store ping --store http://54.153.212.43:8080 
#+end_src
...
- TODO:
- Add efs to make nix-store persistent.
- Create rsync between remote and dev stores
...
- 
* Next steps.
- Go find any iohk discord server for nixops, reading up on https://iohk.io/en/blog/posts/2019/02/06/how-we-use-nix-at-iohk/
- I did find https://discord.com/channels/568306982717751326/570351749848891393 for nixos, but nothing nixops of iohk-nix. Asked a couple of pointers for deploy, got very little back
- Proper exploration of iohk repos and activities. DONE
- Original Goal: (taking detour into iohk repos to see if any of this changes)
## Current back to goal.
- To build our own:  https://github.com/input-output-hk/cardano-node
- Combine https://nixos.org/guides/deploying-nixos-using-terraform.html with https://github.com/yumiai/docs/blob/main/bernoHome/diypool_apply.org
- Make sure deploy of flakes inherit input-output-hk/cardano-node
- Add it all to current nix-ops repo
- Incorporating commands for deploys into devshell commands
- Step 1 use tf framework to stand up the caching server ending with init-script that launches the server
- Step 2 use the above tf structure to incorporate diypool learnings into deploying iohk/cardano-node in our repo running tf with devshell.
